...
Fix a proper type index \( \beta \leq \alpha \) and a \( \beta \)-structural approximation \( \varphi \).
...
We will proceed by recursion along the dependency relation from \cref{def:dependency}; at each stage \( (A, x) \), we assume that we know the value of \( \rho^\star_B(y) \) for all \( (B, y) \prec (A, x) \), and compute \( \rho^\star_A(x) \).
...
The values of \( \rho^\star \) at all addresses that \( x \) depends on can be assembled into a \( \beta \)-structural action \( \psi \).
We will consider its derivative \( \psi_{(A \gg (\gamma > \delta))} \), which is a \( \delta \)-strutural action.
Outside of this recursion, we will be able to show that \( \psi_{(A \gg (\gamma > \delta))} \) is lawful and maps flexible litters to flexible litters as required for \cref{lem:foa:struct_action_to_approx}; for now, if it turned out not to be lawful, we would simply assign
\[ \rho^\star_{A \gg (\gamma > \varepsilon > \bot)}(f_{\delta,\varepsilon}(t)) = f_{\delta,\varepsilon}(t) \]
but this case will never actually occur.
Build a \( \delta \)-structural approximation from \( \psi_{(A \gg (\gamma > \delta))} \) as in \cref{lem:foa:struct_action_to_approx}, and apply the freedom of action theorem at level \( \delta < \gamma \leq \beta \) to obtain a \( \delta \)-allowable permutation \( \rho' \).
We will later be able to show that \( \rho' \) agrees with \( \rho^\star \) on all elements of the support of \( t \).
We set
\[ \rho^\star_{A \gg (\gamma > \varepsilon > \bot)}(f_{\delta,\varepsilon}(t)) = f_{\delta,\varepsilon}(\rho'(t)) \]
